...
function tool(name, description, needsApp) {
    return {
...
function appName(record) { return String(record.appName ?? record.app_name ?? record.name ?? record.application ?? ''); }
...
async function availability(application) {
...
async function reclaimSummary() {
    return withCollections(async (_licenses, decisionsCollection) => {
        ...
        const counts = reclaimable.reduce((result, row) => {
...
async function decisionSummary(application) {
    return withCollections(async (_licenses, decisionsCollection) => {
        const decisions = await decisionsCollection.find(appQuery(application)).limit(200).toArray();
        const categories = decisions.reduce((result, row) => {
...
async function reclaimDetails(application) {
    return withCollections(async (_licenses, decisionsCollection) => {
...
async function callMcpTool(name, args) {
    ...
    if (!text)
        throw new Error(`MCP tool ${name} returned no text.`);
    return text;
}
...
async function answer(messages) {
    ...
    for (let round = 0; round < 3; round += 1) {
        ...
        for (const call of calls) {
            const output = await callMcpTool(call.name, JSON.parse(call.arguments || '{}'));
...
app.post('/api/assistant/chat', async (request, response) => {
    const messages = Array.isArray(request.body?.messages)
        ? request.body.messages.filter((item) => (item?.role === 'user' || item?.role === 'assistant') && typeof item.content === 'string' && item.content.trim()).slice(-10).map((item) => ({ role: item.role, content: item.content.trim().slice(0, 2000) }))
        : [];
    ...
    try {
        sendJson(response, 200, await answer(messages));
    }
    catch (error) {
        console.error('License assistant request failed:', errorMessage(error));
        sendJson(response, 502, { error: 'Unable to prepare a license-monitoring answer right now.' });
    }
});

...
type FigmaDesignResults = DesignBriefResults & {
  figmaDesign: FigmaDesignArtifact;
};

type ReactGenerationResults = FigmaDesignResults & {
  reactGeneration: ReactGenerationResult;
};

type AnalysisResults = ReactGenerationResults & {
...
type MutationResults = AnalysisResults & {
  databasePath: string;
...
function resolveGeneratedArtifactPath(relativePath: string): string {
...
async function createFigmaDesignFromBrief(input: DesignBriefResults): Promise<FigmaDesignResults> {
  ...
  const nodes = [
    { name: 'Restaurant Landing Page Frame', type: 'frame' as const, description: 'Desktop landing page frame generated from Slack prompt.' },
...
async function readDeliveryDatabaseViaMcp(): Promise<DeliveryDatabase> {
...
async function writeDeliveryDatabaseViaMcp(database: DeliveryDatabase): Promise<{ path: string; recordsUpdated: number }> {
  const client = await getDeliveryMcpClient();
  const result = await client.callTool({
...
async function writeGeneratedArtifactViaMcp(file: GeneratedFile): Promise<{ path: string }> {
  const client = await getDeliveryMcpClient();
  const result = await client.callTool({
...
function parseSlackCommandText(textValue: unknown, requesterValue?: unknown): ExecutionContext {
...
function getDeliveryRecordForRequest(requestId: string): DeliveryRecord {
  const database = deliveryDatabaseSchema.parse(JSON.parse(fs.readFileSync(DELIVERY_DATABASE_PATH, 'utf-8')));
  const deliveryRecord = requestId === 'latest'

...
type AuditResults = ExecutionContext & {
  reclaimedUser: string;
  licenseId: string;
  auditStatus: 'reallocated' | 'already_assigned';
  updatedLicenseArray: License[];
  database: LicenseDatabase;
};

type MutationResults = AuditResults & {
  databasePath: string;
...
function daysSince(dateValue: string, now = new Date()): number {
...
async function getLicenseMcpClient(): Promise<Client> {
  if (!mcpClientPromise) {
...
async function initializeLicenseMcpClient(): Promise<Client> {
...
async function readLicenseDatabaseViaMcp(): Promise<LicenseDatabase> {
...
async function writeLicenseDatabaseViaMcp(database: LicenseDatabase): Promise<{ path: string; recordsUpdated: number }> {
  const client = await getLicenseMcpClient();
  const result = await client.callTool({
...
// ---------------------------------------------------------
async function runLicenseAudit(context: ExecutionContext): Promise<AuditResults> {
  ...
  const existingAssignedLicense = matchingLicenses.find(
    (license) => license.assignedTo.toLowerCase() === context.targetUser.toLowerCase()
  );
  ...
  const updatedLicenseArray = database.licenses.map((license) =>
    license.id === reclaimableLicense.id
      ? {
          ...license,
...
// -----------------------------------------------------------------------
async function dispatchSlackInteractiveCard(results: GovernanceResults): Promise<void> {
...
function parseSlackCommandText(textValue: unknown): ExecutionContext {
...
function renderApprovalUi(licenseId: string, request: Request): string {
...
function assistantAppName(record: AssistantRecord): string {
  return String(record.appName ?? record.app_name ?? record.name ?? record.application ?? '');
}

async function callAssistantToolViaMcp(name: string, argumentsValue: Record<string, unknown>): Promise<string> {
  ...
  if (!allowedTools.has(name)) throw new Error(`Assistant requested unsupported MCP tool: ${name}`);
  const client = await getLicenseMcpClient();
  const result = await client.callTool({ name, arguments: argumentsValue });
  const content = result.content as McpTextContent[];
  const textContent = content.find((item) => item.type === 'text');
  if (!textContent) throw new Error(`MCP ${name} returned no text content.`);
  return textContent.text;
}

function assistantDecisionText(record: AssistantRecord): string {
  return [record.status, record.decision, record.recommendation, record.result, record.reason]
    .filter(Boolean)
    .join(' ');
}

function isReclaimableDecision(record: AssistantRecord): boolean {
  return /reclaim|available|unused|inactive|underutilized|release/i.test(assistantDecisionText(record));
}

function assistantNumber(record: AssistantRecord, keys: string[]): number {
...
function assistantAppMatcher(appName: string): RegExp {
  const normalized = appName.trim().replace(/\.exe$/i, '');
  return new RegExp(`^${escapeMongoRegex(normalized)}(?:\\.exe)?$`, 'i');
}

async function withAssistantCollections<T>(
  operation: (licenses: import('mongodb').Collection<AssistantRecord>, decisions: import('mongodb').Collection<AssistantRecord>) => Promise<T>
): Promise<T> {
  ...
  try {
    ...
    return await operation(
      database.collection<AssistantRecord>('onboarded_licenses'),
      database.collection<AssistantRecord>('evaluation_decisions')
    );
  } finally {
...
async function getLicenseAvailabilityMetrics(appName: string): Promise<Record<string, unknown>> {
  return withAssistantCollections(async (licenseCollection, decisionCollection) => {
    ...
    const totalSeats = licenses.reduce(
      (total, record) => total + assistantNumber(record, ['totalSeats', 'total_seats', 'seats', 'quantity']),
      0
    );
    const assignedSeats = licenses.reduce(
      (total, record) => total + assistantNumber(record, ['assignedSeats', 'assigned_seats', 'usedSeats']),
      0
    );
    const explicitlyAvailable = licenses.reduce(
      (total, record) => total + assistantNumber(record, ['availableSeats', 'available_seats']),
      0
    );
...
async function getReclaimableLicenseSummary(): Promise<Record<string, unknown>> {
  return withAssistantCollections(async (_licenseCollection, decisionCollection) => {
    ...
    const byApplication = reclaimable.reduce<Record<string, number>>((counts, record) => {
    ...
    return {
      ...
      byApplication: Object.fromEntries(
        Object.entries(byApplication).sort((first, second) => second[1] - first[1]).slice(0, 20)
      )
    };
  });
}

async function getCompletedDecisionSummary(appName: string): Promise<Record<string, unknown>> {
  return withAssistantCollections(async (_licenseCollection, decisionCollection) => {
    const appMatcher = assistantAppMatcher(appName);
    const decisions = await decisionCollection
      ...
      .limit(200)
      .toArray();
    const categories = decisions.reduce<Record<string, number>>((counts, record) => {
...
async function getReclaimableLicenseDetails(appName: string): Promise<Record<string, unknown>> {
  return withAssistantCollections(async (_licenseCollection, decisionCollection) => {
    const appMatcher = assistantAppMatcher(appName);
    const decisions = await decisionCollection
      .find({ $or: [{ appName: appMatcher }, { app_name: appMatcher }, { name: appMatcher }, { application: appMatcher }] })
...
async function buildFallbackAssistantAnswer(question: string): Promise<{ answer: string; recordsReviewed: number }> {
  const mongoClient = new MongoClient(process.env.MONGO_URI || 'mongodb://localhost:27017');
  try {
    await mongoClient.connect();
    const database = mongoClient.db(process.env.MONGO_DATABASE || 'app-usage-monitoring');
    const [licenses, decisions] = await Promise.all([
      database.collection<AssistantRecord>('onboarded_licenses').find({}).limit(500).toArray(),
      database
        .collection<AssistantRecord>('evaluation_decisions')
        .find({})
        .sort({ completed_date: -1, completed_time: -1 })
        .limit(500)
        .toArray()
    ...
    const appNames = [...new Set(records.map(assistantAppName).filter(Boolean))].sort(
      (first, second) => second.length - first.length
    );
    const requestedApp = appNames.find((name) => normalizedQuestion.includes(name.toLowerCase()));

    if (requestedApp) {
      const matchesApp = (record: AssistantRecord) =>
        assistantAppName(record).toLowerCase() === requestedApp.toLowerCase();
      ...
      const explicitAvailable = appLicenses.reduce(
        (total, record) => total + assistantNumber(record, ['availableSeats', 'available_seats']),
        0
      );
    ...
    if (/reclaim|unused|inactive|underutilized/.test(normalizedQuestion)) {
      const appCounts = reclaimable.reduce<Record<string, number>>((counts, record) => {
        const name = assistantAppName(record) || 'Unknown application';
        counts[name] = (counts[name] || 0) + 1;
        return counts;
      }, {});
...
function openAIText(payload: OpenAIResponsePayload): string {
...
async function createOpenAIResponse(body: Record<string, unknown>): Promise<OpenAIResponsePayload> {
...
function countReviewedToolRecords(toolResult: string): number {
  try {
    const value = JSON.parse(toolResult) as Record<string, unknown>;
    return Number(
      value.completedDecisionsReviewed || value.completedDecisionCount || value.reclaimableDecisionCount || 0
    ) + Number(value.inventoryRecords || 0);
  } catch { return 0; }
}
...
): Promise<{ answer: string; recordsReviewed: number; source: 'openai-mcp' | 'monitoring-agent-fallback' }> {
  if (!process.env.OPENAI_API_KEY?.trim()) {
    return { ...(await buildFallbackAssistantAnswer(question)), source: 'monitoring-agent-fallback' };
  }
  try {
    const inputItems: Array<Record<string, unknown>> = conversation.length
    ...
    for (let round = 0; round < 3; round += 1) {
      ...
      if (calls.length === 0) {
        ...
        if (!answer) throw new Error('OpenAI returned neither an answer nor a tool call');
      ...
      for (const call of calls) {
        ...
        catch { throw new Error(`OpenAI supplied invalid arguments for ${call.name}`); }
      ...
      inputItems.push(...((response.output || []) as Array<Record<string, unknown>>), ...toolOutputs);
...
function startServer() {
  ...
  app.post('/api/assistant/chat', async (request, response) => {
    const messages: Array<{ role: 'user' | 'assistant'; content: string }> = Array.isArray(request.body?.messages)

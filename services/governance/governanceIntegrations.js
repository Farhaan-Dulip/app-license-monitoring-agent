...
export async function createGitHubEvidenceBranch(requester) {
...
export async function provisionGovernanceIntake(context) {
...
export async function syncEvidenceToGitHubBranch(results, owner, repo, branchName) {
    const octokit = getOctokit();
    if (!octokit) {
...
export async function ensureEvidencePullRequest(results, owner, repo, branchName) {
    const octokit = getOctokit();
    if (!octokit) {
...
export async function postGitHubPrReviewComment(owner, repo, prNumber, title, bodyLines) {
    const octokit = getOctokit();
    if (!octokit) {
        return;
    }
...
export async function closeLinearTicketAfterMerge(ticketId, requestId) {
    if (!ticketId) {
...
export async function mergeGitHubPrAfterReview(prOwner, prRepo, prNumberValue, requestId) {

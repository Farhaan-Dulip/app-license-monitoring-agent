import { CachingPubSub } from '../../chunk-M4R3JFZJ.js';
import { runIdleLoop, SaveQueueManager, createStep, mergeProviderOptions, PrepareStepProcessor, composeStepInput, buildLlmPromptArgs, applyAutoResumeSystemMessage, injectBackgroundTaskPrompt, execute, mergeLlmCallHeaders, buildMemoryHeaders, inferProviderExecuted, buildMessagesFromChunks, findProviderToolByName, createWorkflow, pruneAgentLoopSnapshot, runScorer, Agent, runStreamCompletionScorers, formatStreamCompletionFeedback, resolveGoalStore, readObjective, resolveEffectiveGoalSettings, createGoalScorer, GOAL_SCORER_ID, GOAL_SCORE_WAITING, writeObjective } from '../../chunk-77JDY5O7.js';
import { PUBSUB_SYMBOL } from '../../chunk-2QXNHEDL.js';
import { agentThreadStreamRuntime } from '../../chunk-34TKMNFP.js';
import { TripWire, MastraModelOutput, isSupportedLanguageModel, ProcessorRunner, safeClose, safeEnqueue, createProcessorSendSignal } from '../../chunk-7YYAFR2H.js';
import { resolveModelConfig } from '../../chunk-5RBSQIBI.js';
import { EventEmitterPubSub } from '../../chunk-ZBPKFPOE.js';
import { InMemoryServerCache } from '../../chunk-366PLPDH.js';
import { deepMerge } from '../../chunk-SHH6QOJD.js';
import { getNeedsApprovalFn } from '../../chunk-JQJBCTIC.js';
import { toStandardSchema } from '../../chunk-6SRTDZ7S.js';
import { createObservabilityContext } from '../../chunk-TJLLOCZN.js';
import { getOrCreateSpan, EntityType, getStepAvailableToolNames } from '../../chunk-GXMRXZKZ.js';
import { resolveBackgroundConfig, createBackgroundTask } from '../../chunk-RBRYISS2.js';
import { MessageList, normalizeToolPayloadTransformPolicy, mastraDBMessageToSignal, transformToolPayloadForTargets, withToolPayloadTransformMetadata } from '../../chunk-DW27UJIQ.js';
import { RequestContext, MASTRA_VERSIONS_KEY, mergeVersionOverrides } from '../../chunk-W4DLMY73.js';
import { MastraError, ErrorCategory, ErrorDomain } from '../../chunk-HJNLTRHA.js';
...
async function prepareForDurableExecution(options) {
  ...
  if (inputProcessors.length > 0) {
    try {
      const { ProcessorRunner: ProcessorRunner2 } = await import('../../runner-ESYECFE4.js');
    ...
    execute: async (params) => {
      ...
      if (!tool && mastra) {
        ...
        if (rebuilt) {
          ...
          if (!tool) {
            tool = findProviderToolByName(rebuiltTools, toolName);
          }
          if (!tool) {
...
function createDurableGoalStep() {
  return createStep({
    ...
    execute: async (params) => {
      ...
      const waiting = !judgeFailed && !result.complete && result.scorers.some((s) => s.scorerId === GOAL_SCORER_ID && s.score === GOAL_SCORE_WAITING);
...
var DurableAgent = class extends Agent {
  ...
  async resume(runId, resumeData, options) {
    ...
    if (!entry) {
      ...
      const requestContext2 = options?.requestContext ?? (workflowInput.requestContextEntries ? new RequestContext(
  ...
  async recover(runId, options) {
    ...
    const saveQueueManager = memory ? new SaveQueueManager({ logger: this.#mastra?.getLogger?.(), memory }) : void 0;

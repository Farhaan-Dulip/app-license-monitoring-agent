...
var chunkYWVZ3EYR_cjs = require('../chunk-YWVZ3EYR.cjs');
...
function defaultErrorProcessors() {
  return [
    new chunk5R7YROFE_cjs.StreamErrorRetryProcessor({
      retryUnknownErrors: true,
      maxRetries: 2,
      delayMs: 3e3,
      matchers: [
        { match: chunk5R7YROFE_cjs.isBadRequestError, maxRetries: 1, delayMs: 2e3 },
        {
    ...
    new chunk5R7YROFE_cjs.PrefillErrorHandler(),
    new chunk5R7YROFE_cjs.ProviderHistoryCompat()
  ];
}
function defaultWorkspace(basePath) {
  return new chunk5R7YROFE_cjs.Workspace({
    filesystem: new chunk5R7YROFE_cjs.LocalFilesystem({ basePath }),
    sandbox: new chunk5R7YROFE_cjs.LocalSandbox({ workingDirectory: basePath })
  });
}
function createCodingAgent(config) {
  ...
  const taskSignals = memory ? [new chunkYWVZ3EYR_cjs.TaskSignalProvider()] : [];
  const resolvedSignals = signals ? [...signals, ...taskSignals] : taskSignals;
  const resolvedGoal = goal ? { ...goal, prompt: goal.prompt ?? chunk5R7YROFE_cjs.DEFAULT_GOAL_JUDGE_PROMPT } : void 0;
  return new chunk5R7YROFE_cjs.Agent({
    ...rest,
    memory,
    workspace,

...
function splitRuntimeModelString(input) {
  const providers = chunkLIS73MSY_cjs.getRegisteredProviders().sort((a, b) => b.length - a.length);
  ...
  const parsed = chunkLIS73MSY_cjs.parseModelString(input);
...
function isModelAllowed(allowed, candidate) {
  ...
  const activeEntries = allowed.filter((entry) => {
    ...
    return chunkLIS73MSY_cjs.isProviderRegistered(entry.provider);
  });

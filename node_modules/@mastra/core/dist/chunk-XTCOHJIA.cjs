...
function getDisplayTransform(metadata, phase, fallback) {
  const transform = chunkORSCZRC3_cjs.getTransformedToolPayload(metadata, "display", phase);
  return chunkORSCZRC3_cjs.hasTransformedToolPayload(transform) ? transform.transformed : fallback;
}
...
function toUserSignalMessage(payload) {
  ...
  const signal = chunkORSCZRC3_cjs.createSignal({
    id,
    type: "user",
    tagName: "user",
...
function describeServerSideFallback(providerMetadata) {
  const fallback = chunk5R7YROFE_cjs.getServerSideFallbackInfo(providerMetadata);
  if (!fallback) {
    return void 0;
  }
...
var SessionRunEngine = class {
  ...
  async processStreamChunk(state, chunk, requestContext) {
    ...
    switch (chunk.type) {
      ...
      case "tool-call-delta": {
        ...
        const transform = chunkORSCZRC3_cjs.getTransformedToolPayload(chunk.metadata, "display", "input-delta");
        if (!transform?.suppress) {
          this.#session.emit({
            type: "tool_input_delta",
            toolCallId,
            argsTextDelta: chunkORSCZRC3_cjs.hasTransformedToolPayload(transform) ? transform.transformed : argsTextDelta,
            toolName
          });
        }
        break;
      ...
      case "tool-call-approval": {
        ...
        const approvalTransform = chunkORSCZRC3_cjs.getTransformedToolPayload(chunk.metadata, "display", "approval");
        const toolArgs = chunkORSCZRC3_cjs.hasTransformedToolPayload(approvalTransform) ? approvalTransform.transformed : getDisplayTransform(chunk.metadata, "input-available", chunk.payload.args);
      ...
      case "error": {
        const streamError = chunkZWNRICII_cjs.getErrorFromUnknown(chunk.payload.error);
  ...
  async handleSubscribedStreamError(error) {
    if (error instanceof Error && error.name === "AbortError") {
      ...
      this.#session.emit({ type: "error", error: chunkZWNRICII_cjs.getErrorFromUnknown(error) });
...
var SessionOMRole = class {
  ...
  resolvedModel() {
    ...
    return new chunkOBB3OVBT_cjs.ModelRouterLanguageModel(modelId, this.#gateways);
  }
  ...
  apply(event) {
    ...
    switch (event.type) {
      ...
      case "tool_update": {
        ...
        if (tool) {
          tool.partialResult = typeof event.partialResult === "string" ? event.partialResult : chunkHEE7C2GC_cjs.safeStringify(event.partialResult);
        }
        break;
      }
      case "tool_end": {
...
var Session = class {
  ...
  constructor({
    ...
    if (!workspace || !(workspace instanceof chunk5R7YROFE_cjs.Workspace)) {
...
${fence}` };
  ...
  async respondToToolSuspension({
    ...
    try {
      ...
      const err = chunkZWNRICII_cjs.getErrorFromUnknown(error);
...
function createSubagentTool(opts) {
  ...
  return chunkMNNNQBVN_cjs.createTool({
    id: "subagent",
...
${FORKED_SUBAGENT_TASK_NOTICE}`;
        ...
        if (context?.requestContext) {
          subagentRequestContext = new chunkD2FOXUYM_cjs.RequestContext(context.requestContext.entries());
          if (controllerCtx) {
        ...
        subagentToRun = new chunk5R7YROFE_cjs.Agent({
        ...
        const allWorkspaceToolNames = workspace && allowedWs ? new Set(
          Object.keys(
            await chunk5R7YROFE_cjs.createWorkspaceTools(workspace, {
...
function patchTaskToolForFork(tool, toolId) {
  const stubExecute = async () => {
    ...
    if (toolId === "task_check") {
      const taskCheck = chunk5R7YROFE_cjs.summarizeTaskCheck([]);
      return {
        ...baseResult,
...
var AgentController = class {
  ...
  constructor(config) {
    ...
    this.#gatewayManager = new chunkQK6OG7ZJ_cjs.GatewayManager([...config.gateways ?? [], ...chunkQK6OG7ZJ_cjs.defaultGateways]);
  ...
  async #createSessionForResource(ownerId, id, effectiveResourceId, tags, overrides) {
    const requestContext = overrides?.requestContext ?? new chunkD2FOXUYM_cjs.RequestContext();
    ...
    if (workspaceToConnect && workspaceToConnect instanceof chunk5R7YROFE_cjs.Workspace) {
      try {
        ...
        const initError = chunkZWNRICII_cjs.getErrorFromUnknown(error);
  ...
  async resolveConfiguredMemory() {
    ...
    const memory = typeof configuredMemory === "function" ? await configuredMemory({ requestContext: new chunkD2FOXUYM_cjs.RequestContext(), mastra: this.getMastra() }) : configuredMemory;
    if (!memory) {
  ...
  async runInit() {
    if (this.config.storage && !this.#externalMastra) {
      ...
      this.#internalMastra = new chunk5R7YROFE_cjs.Mastra({
        logger: false,
    ...
    if (this.config.workspace && !this.workspaceInitialized && typeof this.workspace !== "function") {
      try {
        if (!this.workspace) {
          this.workspace = new chunk5R7YROFE_cjs.Workspace(this.config.workspace);
        }
  ...
  getAgentForMode(mode) {
    ...
    if (!this.#legacyAgentMode[mode.id]) {
      ...
      this.#legacyAgentMode[mode.id] = new chunk5R7YROFE_cjs.Agent({
        id: `${this.id}-agent`,
...
` : "";
  ...
  convertToControllerMessage(msg) {
    ...
    if (msg.role === "signal") {
      const signal = chunkORSCZRC3_cjs.mastraDBMessageToSignal(msg);
      if (signal.type === "user") {
    ...
    for (const part of msg.content.parts) {
      switch (part.type) {
        ...
        case "file": {
          const { mediaType, data } = chunkORSCZRC3_cjs.resolveFilePartMediaTypeAndData(part);
  ...
  async buildToolsets(session, requestContext) {
    const builtInTools = {
      ask_user: chunk5R7YROFE_cjs.askUserTool,
      submit_plan: chunk5R7YROFE_cjs.submitPlanTool,
      task_write: chunk5R7YROFE_cjs.taskWriteTool,
      task_update: chunk5R7YROFE_cjs.taskUpdateTool,
      task_complete: chunk5R7YROFE_cjs.taskCompleteTool,
      task_check: chunk5R7YROFE_cjs.taskCheckTool
    };
...
//# sourceMappingURL=chunk-XTCOHJIA.cjs.map
//# sourceMappingURL=chunk-XTCOHJIA.cjs.map
...
var chunkHNRFQPHO_cjs = require('./chunk-HNRFQPHO.cjs');
    ...
    outputStream = new ReadableStream({
      async start(controller) {
        try {
          ...
          chunkOBB3OVBT_cjs.attachModelStreamTransport(outputStream, chunkOBB3OVBT_cjs.readModelStreamTransport(stream));
  ...
  async validateValue(value) {
    ...
    if (this.isZodSchema(this.schema)) {
      try {
        ...
        return {
          success: false,
          error: new chunkZWNRICII_cjs.MastraError(
            {
...
var ObjectFormatHandler = class extends BaseFormatHandler {
  ...
  async processPartialChunk({
    ...
    const { value: currentObjectJson, state } = await chunkHNRFQPHO_cjs.parsePartialJson(processedAccumulatedText);
    if (this.validatePartialChunks && this.partialSchema) {
      const result = this.partialSchema?.safeParse(currentObjectJson);
      if (result.success && result.data && result.data !== void 0 && !chunkHNRFQPHO_cjs.isDeepEqualData(previousObject, result.data)) {
        return {
          shouldEmit: true,
    ...
    if (currentObjectJson !== void 0 && currentObjectJson !== null && typeof currentObjectJson === "object" && !chunkHNRFQPHO_cjs.isDeepEqualData(previousObject, currentObjectJson)) {
      return {
  ...
  async validateAndTransformFinal(finalRawValue) {
    ...
    const { value } = await chunkHNRFQPHO_cjs.parsePartialJson(rawValue);
...
var ArrayFormatHandler = class extends BaseFormatHandler {
  ...
  async processPartialChunk({
    ...
    const processedAccumulatedText = this.preprocessText(accumulatedText);
    const { value: currentObjectJson, state: parseState } = await chunkHNRFQPHO_cjs.parsePartialJson(processedAccumulatedText);
    if (currentObjectJson !== void 0 && !chunkHNRFQPHO_cjs.isDeepEqualData(previousObject, currentObjectJson)) {
      ...
      if (!chunkHNRFQPHO_cjs.isDeepEqualData(this.textPreviousFilteredArray, filteredElements)) {
...
var EnumFormatHandler = class extends BaseFormatHandler {
  ...
  async processPartialChunk({
    ...
    const processedAccumulatedText = this.preprocessText(accumulatedText);
    const { value: currentObjectJson } = await chunkHNRFQPHO_cjs.parsePartialJson(processedAccumulatedText);
    if (currentObjectJson !== void 0 && currentObjectJson !== null && typeof currentObjectJson === "object" && !Array.isArray(currentObjectJson) && "result" in currentObjectJson && typeof currentObjectJson.result === "string" && !chunkHNRFQPHO_cjs.isDeepEqualData(previousObject, currentObjectJson)) {
  ...
  async validateAndTransformFinal(rawFinalValue) {
    ...
    const { value } = await chunkHNRFQPHO_cjs.parsePartialJson(processedValue);
...
var MastraModelOutput = class extends chunkWSD4JNMB_cjs.MastraBase {
  ...
  constructor({
    ...
    const resultSpan = chunkAHAX7NUG_cjs.getRootExportSpan(options.tracingContext?.currentSpan);
          ...
          async transform(chunk, controller) {
            if (chunk.type === "finish" && chunk.payload?.stepResult?.reason === "tool-calls") {
              ...
              const {
                ...
                chunk,
                processorStates,
                chunkLCYW7LNO_cjs.resolveObservabilityContext(options),
            ...
            case "finish":
              ...
              try {
                if (self.processorRunner && !self.#options.isLLMExecutionStep) {
                  ...
                  const outputText = lastResponseMessage ? chunkORSCZRC3_cjs.coreContentToString(lastResponseMessage.content) : "";
                ...
                if (error2 instanceof TripWire) {
                  ...
                  self.#error = chunkZWNRICII_cjs.getErrorFromUnknown(error2, {
              ...
              break;
            case "error":
              const error = chunkZWNRICII_cjs.getErrorFromUnknown(chunk.payload.error, {
  ...
  resolvePromise(key, value) {
    if (!(key in this.#delayedPromises)) {
      throw new chunkZWNRICII_cjs.MastraError({
        ...
        domain: chunkZWNRICII_cjs.ErrorDomain.LLM,
        category: chunkZWNRICII_cjs.ErrorCategory.SYSTEM,
...
var getModelOutputForTripwire = async ({
  ...
  const observabilityContext = chunkLCYW7LNO_cjs.resolveObservabilityContext(rest);
...
function isStructuredOutputFormatError(error) {
  return chunkUNBULUZP_cjs.JSONParseError.isInstance(error) || chunkHNRFQPHO_cjs.NoObjectGeneratedError.isInstance(error) || chunkUNBULUZP_cjs.TypeValidationError.isInstance(error) || error instanceof chunkZWNRICII_cjs.MastraError && error.id === "STRUCTURED_OUTPUT_OBJECT_UNDEFINED";
}
...
function createProcessorSendSignal(args) {
  return async (signalInput) => {
    const signal = chunkORSCZRC3_cjs.createSignal(signalInput);
...
var ProcessorState = class {
  ...
  constructor(options) {
    ...
    this.span = parentSpan?.createChildSpan({
      ...
      entityType: chunkAHAX7NUG_cjs.EntityType.OUTPUT_PROCESSOR,
...
function areProcessorMessageArraysEqual(before, after) {
  ...
  return before.length === after.length && before.every((message, index) => chunkORSCZRC3_cjs.messagesAreEqual(message, after[index]));
}
...
var ProcessorRunner = class _ProcessorRunner {
  ...
  async runComputeStateSignal({
    ...
    const trackingById = chunkKCEBVWSP_cjs.getStateSignalsMetadata(thread.metadata);
    const tracking = trackingById[stateId];
    const { activeStateSignals, contextWindow, lastSnapshot, deltasSinceSnapshot } = await chunkKCEBVWSP_cjs.resolveStateSignalHistory({
      messageList,
    ...
    const result = await computeStateSignal({
      ...
      sendStateSignal: async (stateSignal) => {
        const sendResult = await chunkKCEBVWSP_cjs.applyStateSignal({
          input: stateSignal,
          memory: resolvedMemory,
          thread,
          resourceId: resolvedResourceId,
    ...
    if (!result) return;
    await chunkKCEBVWSP_cjs.applyStateSignal({
      input: result,
      memory: resolvedMemory,
      thread,
      resourceId: resolvedResourceId,
    ...
    for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
      ...
      try {
        ...
        const processResult = await processMethod({
          ...
          abort,
          agent: this.agent,
          ...chunkLCYW7LNO_cjs.createObservabilityContext({ currentSpan: processorSpan }),
          requestContext,
          retryCount,
          writer,
        ...
        if (processResult instanceof chunkORSCZRC3_cjs.MessageList) {
        ...
        try {
          if (processor.processOutputStream && processedPart) {
            ...
            const result = await processor.processOutputStream({
              ...
              ...chunkLCYW7LNO_cjs.createObservabilityContext({ currentSpan: state.span }),
              requestContext,
              messageList,
              retryCount,
              writer
  ...
  async runInputProcessors(messageList, observabilityContext, requestContext, retryCount = 0) {
    for (const [index, processorOrWorkflow] of this.inputProcessors.entries()) {
      ...
      const processorSpan = parentSpan?.createChildSpan({
        ...
        entityType: chunkAHAX7NUG_cjs.EntityType.INPUT_PROCESSOR,
        entityId: processor.id,
        entityName: processor.name,
        attributes: {
          processorExecutor: "legacy",
      ...
      try {
        ...
        let mutations;
        if (result instanceof chunkORSCZRC3_cjs.MessageList) {
  ...
  async runProcessInputStep(args) {
    ...
    const observabilityContext = chunkLCYW7LNO_cjs.resolveObservabilityContext(args);
    const stepInput = {
    ...
    for (const [index, processorOrWorkflow] of processors.entries()) {
      ...
      const processorSpan = currentSpan?.createChildSpan({
        ...
        entityType: chunkAHAX7NUG_cjs.EntityType.INPUT_STEP_PROCESSOR,
        entityId: processor.id,
        entityName: processor.name,
        attributes: {
          processorExecutor: "legacy",
      ...
      try {
        ...
        const processMethodArgs = {
          ...
          sendStateSignal: async (stateSignal) => {
            ...
            const result2 = await chunkKCEBVWSP_cjs.applyStateSignal({
              input: stateSignal,
              memory: resolvedMemory,
              thread,
              resourceId: resolvedResourceId,
  ...
  async runProcessLLMRequest(args) {
    chunkLCYW7LNO_cjs.resolveObservabilityContext({ tracingContext: args.tracingContext });
    ...
    for (const processorOrWorkflow of this.inputProcessors) {
      ...
      try {
        ...
        const result = await processMethod({
          ...
          abortSignal: args.abortSignal,
          writer: args.writer,
          ...chunkLCYW7LNO_cjs.createObservabilityContext(args.tracingContext)
        });
  ...
  async runProcessOutputStep(args) {
    ...
    for (const [index, processorOrWorkflow] of this.outputProcessors.entries()) {
      ...
      const processorSpan = parentSpan?.createChildSpan({
        ...
        entityType: chunkAHAX7NUG_cjs.EntityType.OUTPUT_STEP_PROCESSOR,
        entityId: processor.id,
        entityName: processor.name,
        attributes: {
          processorExecutor: "legacy",
  ...
  static async validateAndFormatProcessInputStepResult(result, {
    ...
    if (result instanceof chunkORSCZRC3_cjs.MessageList) {
      ...
      if (result.model) {
        const resolvedModel = await chunkOBB3OVBT_cjs.resolveModelConfig(result.model);
...
//# sourceMappingURL=chunk-E7BWKNX2.cjs.map
//# sourceMappingURL=chunk-E7BWKNX2.cjs.map
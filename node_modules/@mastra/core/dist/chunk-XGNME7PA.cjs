...
var chunkKCEBVWSP_cjs = require('./chunk-KCEBVWSP.cjs');
...
var ObservabilityStorage = class _ObservabilityStorage extends chunkFJFMJDFF_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "OBSERVABILITY"
  ...
  async createSpan(_args) {
    throw new chunkZWNRICII_cjs.MastraError({
      ...
      domain: chunkZWNRICII_cjs.ErrorDomain.MASTRA_OBSERVABILITY,
  ...
  async getBranch(args) {
    const parsed = chunkXMQFQRE2_cjs.getBranchArgsSchema.parse(args);
    try {
      const skeleton = await this.getStructure({ traceId: parsed.traceId });
      if (!skeleton) return null;
      const branchSpanIds = chunkXMQFQRE2_cjs.extractBranchSpans(skeleton.spans, parsed.spanId, parsed.depth).map((s) => s.spanId);
      ...
      const isFallbackTrigger = error instanceof chunkZWNRICII_cjs.MastraError && (error.id === "OBSERVABILITY_STORAGE_GET_STRUCTURE_NOT_IMPLEMENTED" || error.id === "OBSERVABILITY_STORAGE_GET_TRACE_LIGHT_NOT_IMPLEMENTED" || error.id === "OBSERVABILITY_STORAGE_GET_SPANS_NOT_IMPLEMENTED");
      if (!isFallbackTrigger) throw error;
    }
    const trace = await this.getTrace({ traceId: parsed.traceId });
    if (!trace) return null;
    const spans = chunkXMQFQRE2_cjs.extractBranchSpans(trace.spans, parsed.spanId, parsed.depth);
...
function transformRow(row, tableName, options = {}) {
  ...
  const tableSchema = chunkXMQFQRE2_cjs.TABLE_SCHEMAS[tableName];
  const result = {};
...
function transformScoreRow(row, options = {}) {
  return transformRow(row, chunkXMQFQRE2_cjs.TABLE_SCORERS, options);
}
...
var ObservabilityInMemory = class extends ObservabilityStorage {
  ...
  upsertByIdField(records, cursorIds, record, idField) {
    ...
    if (id == null) {
      throw new chunkZWNRICII_cjs.MastraError({
        ...
        domain: chunkZWNRICII_cjs.ErrorDomain.STORAGE,
  ...
  maybeRegisterBranchCursor(span) {
    if (!chunkXMQFQRE2_cjs.BRANCH_SPAN_TYPE_SET.has(span.spanType)) {
      return;
    }
  ...
  getMatchingRootSpans(args) {
    const { filters, pagination, orderBy } = chunkXMQFQRE2_cjs.listTracesArgsSchema.parse(args);
  ...
  async listTraces(args) {
    const { mode, filters, after, limit } = chunkXMQFQRE2_cjs.listTracesArgsSchema.parse(args);
    if (mode === "delta") {
      ...
      return {
        spans: chunkXMQFQRE2_cjs.toTraceSpans(deltaResponse.rows),
    ...
    return {
      spans: chunkXMQFQRE2_cjs.toTraceSpans(paged),
  ...
  async listBranches(args) {
    const { mode, filters, pagination, orderBy, after, limit } = chunkXMQFQRE2_cjs.listBranchesArgsSchema.parse(args);
    if (mode === "delta") {
      this.assertDeltaPollingEnabled();
      ...
      return {
        branches: deltaResponse.rows.map(chunkXMQFQRE2_cjs.toTraceSpan),
        delta: deltaResponse.delta,
        deltaCursor: deltaResponse.deltaCursor
      };
    }
    const allowedSpanTypes = filters?.spanType ? chunkXMQFQRE2_cjs.BRANCH_SPAN_TYPE_SET.has(filters.spanType) ? /* @__PURE__ */ new Set([filters.spanType]) : /* @__PURE__ */ new Set() : chunkXMQFQRE2_cjs.BRANCH_SPAN_TYPE_SET;
    const matches = [];
    for (const [, traceEntry] of this.db.traces) {
    ...
    return {
      ...
      branches: paged.map(chunkXMQFQRE2_cjs.toTraceSpan),
  ...
  spanMatchesBranchFilters(span, filters) {
    ...
    if (filters.status !== void 0) {
      const spanStatus = chunkXMQFQRE2_cjs.toTraceSpan(span).status;
  ...
  async listMetrics(args) {
    const { mode, filters, pagination, orderBy, after, limit } = chunkAHAX7NUG_cjs.listMetricsArgsSchema.parse(args);
    if (mode === "delta") {
      this.assertDeltaPollingEnabled();
  ...
  *iterateObservabilityContextRecords() {
  ...
  async getEntityTypes(_args) {
    const validTypes = new Set(Object.values(chunkAHAX7NUG_cjs.EntityType));
  ...
  async listLogs(args) {
    const { mode, filters, pagination, orderBy, after, limit } = chunkAHAX7NUG_cjs.listLogsArgsSchema.parse(args);
    if (mode === "delta") {
      this.assertDeltaPollingEnabled();
      const deltaResponse = this.listAppendOnlyDelta(
        this.db.logRecords,
  ...
  async listScores(args) {
    const { mode, filters, pagination, orderBy, after, limit } = chunkAHAX7NUG_cjs.listScoresArgsSchema.parse(args);
    if (mode === "delta") {
      this.assertDeltaPollingEnabled();
      const deltaResponse = this.listAppendOnlyDelta(
  ...
  async listFeedback(args) {
    const { mode, filters, pagination, orderBy, after, limit } = chunkAHAX7NUG_cjs.listFeedbackArgsSchema.parse(args);
    if (mode === "delta") {
      this.assertDeltaPollingEnabled();
      const deltaResponse = this.listAppendOnlyDelta(
...
var entityTypeValues = new Set(Object.values(chunkAHAX7NUG_cjs.EntityType));
...
var BackgroundTasksStorage = class extends chunkFJFMJDFF_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
...
var ChannelsStorage = class extends chunkFJFMJDFF_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "CHANNELS"
...
// src/storage/domains/datasets/identity.ts
...
var DatasetItemIdentityConflictError = class extends chunkZWNRICII_cjs.MastraError {
  conflicts;
...
function planDatasetItemBatch(items, historyRows, createId) {
  ...
  for (const [index, item] of items.entries()) {
    if (!item.externalId) {
      const insert2 = { id: createId(), item };
      inserts.push(insert2);
      resolvedIds.push(insert2.id);
      continue;
    }
...
var DATASET_IMMUTABLE_FIELDS = ["organizationId", "projectId", "candidateKey", "candidateId"];
var DatasetsStorage = class extends chunkFJFMJDFF_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "DATASETS"
...
function matchesTenancy(record, filters) {
  if (!filters) return true;
...
var DatasetsInMemory = class extends DatasetsStorage {
  ...
  async listDatasets(args) {
    ...
    const perPage = chunkFJFMJDFF_cjs.normalizePerPage(perPageInput, 100);
    const { offset: start, perPage: perPageForResponse } = chunkFJFMJDFF_cjs.calculatePagination(page, perPageInput, perPage);
  ...
  async _doBatchInsertItems(input) {
    ...
    for (const [index, item] of input.items.entries()) {
      if (!item.externalId) {
        ...
        planned.set(id2, { id: id2, item });
        resolvedIds.push(id2);
        continue;
      }
...
var ExperimentsStorage = class extends chunkFJFMJDFF_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "EXPERIMENTS"
...
var ExperimentsInMemory = class extends ExperimentsStorage {
  ...
  async getExperimentById(args) {
    ...
    if (!row) return null;
    if (args.filters?.organizationId !== void 0 && (row.organizationId ?? null) !== args.filters.organizationId) {
      return null;
    }
    if (args.filters?.projectId !== void 0 && (row.projectId ?? null) !== args.filters.projectId) {
      return null;
    }
    return row;
  }
  ...
  async deleteExperiment(args) {
    ...
    if (!existing) return;
    if (args.filters?.organizationId !== void 0 && (existing.organizationId ?? null) !== args.filters.organizationId) {
      return;
    }
    if (args.filters?.projectId !== void 0 && (existing.projectId ?? null) !== args.filters.projectId) {
      return;
    }
  ...
  async deleteExperimentResults(args) {
    if (args.filters?.organizationId !== void 0 || args.filters?.projectId !== void 0) {
      ...
      if (!parent) return;
      if (args.filters?.organizationId !== void 0 && (parent.organizationId ?? null) !== args.filters.organizationId) {
        return;
      }
      if (args.filters?.projectId !== void 0 && (parent.projectId ?? null) !== args.filters.projectId) {
        return;
      }
    }
    for (const [resultId, result] of this.db.experimentResults) {
...
var HarnessStorage = class extends chunkFJFMJDFF_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "HARNESS"
...
var MemoryStorage = class extends chunkFJFMJDFF_cjs.StorageDomain {
  /**
...
var InMemoryMemory = class extends MemoryStorage {
  ...
  async listMessages({
    ...
    const perPage = chunkFJFMJDFF_cjs.normalizePerPage(perPageInput, 40);
    if (page < 0) {
      throw new Error("page must be >= 0");
    }
    ...
    const { offset, perPage: perPageForResponse } = chunkFJFMJDFF_cjs.calculatePagination(page, perPageInput, perPage);
  ...
  async listMessagesByResourceId({
    ...
    const list = new chunkORSCZRC3_cjs.MessageList().add(
  ...
  async saveMessages(args) {
    ...
    const list = new chunkORSCZRC3_cjs.MessageList().add(messages, "memory");
    return { messages: list.get.all.db() };
  }
...
var defaultAgentThreadPubSub = new chunkZZYMSXJH_cjs.EventEmitterPubSub();
...
var AgentThreadStreamRuntime = class {
  ...
  #getLeaseProvider(pubsub) {
    ...
    if (typeof unwrap === "function") {
      ...
      return inner ?? chunkZZYMSXJH_cjs.NoopLeaseProvider;
    }
    return chunkZZYMSXJH_cjs.isLeaseProvider(resolved) ? resolved : chunkZZYMSXJH_cjs.NoopLeaseProvider;
  }
  #getSourceId() {
  ...
  #getThreadTarget(options) {
    ...
    const threadId = options?.requestContext?.get(chunkD2FOXUYM_cjs.MASTRA_THREAD_ID_KEY) || (typeof thread === "string" ? thread : thread?.id);
    const resourceId = options?.requestContext?.get(chunkD2FOXUYM_cjs.MASTRA_RESOURCE_ID_KEY) || options?.memory?.resource;
  ...
  resetForTests() {
    ...
    defaultAgentThreadPubSub = new chunkZZYMSXJH_cjs.EventEmitterPubSub();
  }
  #resetState(pubsub) {
  ...
  #startContinuation(state, pubsub, key, pending) {
    ...
    void pending.agent.stream(pending.messages, {
      ...
      this.#publish(pubsub, key, {
        type: "run-failed",
        runId: pending.runId,
        error: chunkZWNRICII_cjs.getErrorFromUnknown(err).message
      });
  ...
  async subscribeToThread(agent, options, pubsub) {
    ...
    const onEvent = (event) => {
      ...
      if (data.type === "signal-enqueued") {
        ...
        const queue = signalsByThread.get(key) ?? [];
        queue.push(chunkORSCZRC3_cjs.createSignal(data.signal));
  ...
  sendMessage(agent, message, target, pubsub) {
    return this.sendSignal(agent, chunkORSCZRC3_cjs.createMessageSignal(message, { acceptedAt: /* @__PURE__ */ new Date() }), target, pubsub);
  }
  queueMessage(agent, message, target, pubsub) {
    const state = this.#getState(pubsub);
    const signal = chunkORSCZRC3_cjs.createMessageSignal(message, { acceptedAt: /* @__PURE__ */ new Date() });
    let key;
  ...
  async sendStateSignal(agent, stateInput, target, pubsub) {
    ...
    const memoryContext = chunkKCEBVWSP_cjs.parseMemoryRequestContext(requestContext);
    const memory = await agent.getMemory({ requestContext });
    if (!memory) {
    ...
    const applied = await chunkKCEBVWSP_cjs.applyStateSignal({
      input: stateInput,
      memory,
      thread,
      resourceId,
  ...
  sendSignal(agent, signalInput, target, pubsub) {
    const state = this.#getState(pubsub);
    let signal = chunkORSCZRC3_cjs.createSignal({ ...signalInput, acceptedAt: /* @__PURE__ */ new Date() });
    let key;
    let runId = target.runId;
    ...
    const resourceId = target.resourceId ?? activeRecord?.resourceId;
    const threadId = target.threadId ?? activeRecord?.threadId;
    signal = chunkORSCZRC3_cjs.resolveDeliveryAttributes(
      signal,
    ...
    const accepted = (async () => {
      ...
      try {
        ...
        this.#publish(pubsub, reservedKey, {
          type: "run-failed",
          runId: reservedRunId,
          error: chunkZWNRICII_cjs.getErrorFromUnknown(error).message
        });
...
function createNotificationSignal(notification) {
  return chunkORSCZRC3_cjs.createSignal({
    type: "notification",
...
var NotificationsStorage = class extends chunkFJFMJDFF_cjs.StorageDomain {
  constructor() {
...
var SchedulesStorage = class extends chunkFJFMJDFF_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "SCHEDULES"
...
var ScoresStorage = class extends chunkFJFMJDFF_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "SCORES"
...
function matchesTenancy2(score, filters) {
  if (!filters) return true;
  if (filters.organizationId !== void 0 && score.organizationId !== filters.organizationId) return false;
...
var ScoresInMemory = class extends ScoresStorage {
  ...
  async listScoresByScorerId({
    ...
    const scores = Array.from(this.db.scores.values()).filter((score) => {
      ...
      return baseFilter && matchesTenancy2(score, filters);
    });
    scores.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    const { page, perPage: perPageInput } = pagination;
    const perPage = chunkFJFMJDFF_cjs.normalizePerPage(perPageInput, Number.MAX_SAFE_INTEGER);
    const { offset: start, perPage: perPageForResponse } = chunkFJFMJDFF_cjs.calculatePagination(page, perPageInput, perPage);
  ...
  async listScoresByRunId({
    ...
    const scores = Array.from(this.db.scores.values()).filter(
      (score) => score.runId === runId && matchesTenancy2(score, filters)
    );
    const { page, perPage: perPageInput } = pagination;
    const perPage = chunkFJFMJDFF_cjs.normalizePerPage(perPageInput, Number.MAX_SAFE_INTEGER);
    const { offset: start, perPage: perPageForResponse } = chunkFJFMJDFF_cjs.calculatePagination(page, perPageInput, perPage);
  ...
  async listScoresBySpan({
    ...
    const scores = Array.from(this.db.scores.values()).filter(
      (score) => score.traceId === traceId && score.spanId === spanId && matchesTenancy2(score, filters)
    );
    scores.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    const { page, perPage: perPageInput } = pagination;
    const perPage = chunkFJFMJDFF_cjs.normalizePerPage(perPageInput, Number.MAX_SAFE_INTEGER);
...
var WorkflowsStorage = class extends chunkFJFMJDFF_cjs.StorageDomain {
  constructor() {
    super({
      component: "STORAGE",
      name: "WORKFLOWS"
...
var WorkflowsInMemory = class extends WorkflowsStorage {
  ...
  async listWorkflowRuns({
    ...
    if (perPage !== void 0 && page !== void 0) {
      const normalizedPerPage = chunkFJFMJDFF_cjs.normalizePerPage(perPage, Number.MAX_SAFE_INTEGER);
...
// src/storage/mock.ts
var InMemoryStore = class extends chunkFJFMJDFF_cjs.MastraCompositeStore {
  stores;
  /**
  ...
  constructor({ id = "in-memory" } = {}) {
    ...
    this.#db = new chunk5G75F3KP_cjs.InMemoryDB();
    this.stores = {
      ...
      agents: new chunk5G75F3KP_cjs.InMemoryAgentsStorage({ db: this.#db }),
      ...
      promptBlocks: new chunk2L63IHLZ_cjs.InMemoryPromptBlocksStorage({ db: this.#db }),
      scorerDefinitions: new chunkOPL4PX3Z_cjs.InMemoryScorerDefinitionsStorage({ db: this.#db }),
      mcpClients: new chunkCJUJT4JS_cjs.InMemoryMCPClientsStorage({ db: this.#db }),
      mcpServers: new chunkDONWL3I6_cjs.InMemoryMCPServersStorage({ db: this.#db }),
      workspaces: new chunkLZS6RUAP_cjs.InMemoryWorkspacesStorage({ db: this.#db }),
      skills: new chunkIZEFW2YY_cjs.InMemorySkillsStorage({ db: this.#db }),
      ...
      threadState: new chunkFJFMJDFF_cjs.InMemoryThreadStateStorage()
    };
  }
  /**
...
var FilesystemStore = class extends chunkFJFMJDFF_cjs.MastraCompositeStore {
  #db;
  #dir;
  constructor(config = {}) {
    ...
    this.stores = {
      agents: new chunk5G75F3KP_cjs.FilesystemAgentsStorage({ db: this.#db }),
      promptBlocks: new chunk2L63IHLZ_cjs.FilesystemPromptBlocksStorage({ db: this.#db }),
      scorerDefinitions: new chunkOPL4PX3Z_cjs.FilesystemScorerDefinitionsStorage({ db: this.#db }),
      mcpClients: new chunkCJUJT4JS_cjs.FilesystemMCPClientsStorage({ db: this.#db }),
      mcpServers: new chunkDONWL3I6_cjs.FilesystemMCPServersStorage({ db: this.#db }),
      workspaces: new chunkLZS6RUAP_cjs.FilesystemWorkspacesStorage({ db: this.#db }),
      skills: new chunkIZEFW2YY_cjs.FilesystemSkillsStorage({ db: this.#db })
    };
  }
  /**
...
var StoreOperationsInMemory = class extends StoreOperations {
  ...
  async insert({ tableName, record }) {
    ...
    let key = record.id;
    if (tableName === chunkXMQFQRE2_cjs.TABLE_NOTIFICATIONS && record.threadId && record.id) {
      key = `${record.threadId}\0${record.id}`;
    } else if ([chunkXMQFQRE2_cjs.TABLE_WORKFLOW_SNAPSHOT].includes(tableName) && !record.id && record.run_id) {
...
exports.DatasetItemIdentityConflictError = DatasetItemIdentityConflictError;
...
exports.normalizeScheduleTarget = normalizeScheduleTarget;
...
//# sourceMappingURL=chunk-XGNME7PA.cjs.map
//# sourceMappingURL=chunk-XGNME7PA.cjs.map
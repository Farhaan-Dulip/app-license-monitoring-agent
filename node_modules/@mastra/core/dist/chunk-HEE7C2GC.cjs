...
function mergeRequestContexts(closureRC, execRC) {
  ...
  if (!closureRC && !execRC) return new chunkD2FOXUYM_cjs.RequestContext();
  if (!closureRC) return execRC instanceof chunkD2FOXUYM_cjs.RequestContext ? execRC : new chunkD2FOXUYM_cjs.RequestContext();
  if (!execRC || !(execRC instanceof chunkD2FOXUYM_cjs.RequestContext) || execRC.size() === 0) return closureRC;
  const merged = new chunkD2FOXUYM_cjs.RequestContext();
...
var CoreToolBuilder = class extends chunkWSD4JNMB_cjs.MastraBase {
  ...
  constructor(input) {
    ...
    if (!chunkMNNNQBVN_cjs.isVercelTool(this.originalTool) && !chunkMNNNQBVN_cjs.isProviderDefinedTool(this.originalTool)) {
        ...
        if (chunkCMKSC37Z_cjs.isZodObject(schema) && isZodV4Schema(schema)) {
          ...
          if (isBackgroundEligible) {
            nextSchema = chunkCMKSC37Z_cjs.safeExtendZodObject(nextSchema, {
              _background: chunkH4SAIERM_cjs.backgroundOverrideZodSchema
            });
          }
          if (isResumableTool) {
            nextSchema = chunkCMKSC37Z_cjs.safeExtendZodObject(nextSchema, {
          ...
          if (jsonSchema2 && typeof jsonSchema2 === "object" && jsonSchema2.type === "object") {
            ...
            const injectedKeys = [];
            if (isBackgroundEligible) {
              properties._background = chunkH4SAIERM_cjs.backgroundOverrideJsonSchema;
  ...
  getParameters = () => {
    if (chunkMNNNQBVN_cjs.isVercelTool(this.originalTool)) {
  ...
  createExecute(tool, options, logType) {
    ...
    const mcpMeta = !chunkMNNNQBVN_cjs.isVercelTool(tool) && "mcpMetadata" in tool ? tool.mcpMetadata : void 0;
    const execFunction = async (args, execOptions, toolSpan) => {
      try {
        let result;
        let suspendData = null;
        if (chunkMNNNQBVN_cjs.isVercelTool(tool)) {
          result = await chunkAHAX7NUG_cjs.executeWithContext({
            span: toolSpan,
            fn: async () => tool?.execute?.(args, execOptions)
          });
        } else {
          const wrappedMastra = options.mastra ? chunkLCYW7LNO_cjs.wrapMastra(options.mastra, { currentSpan: toolSpan }) : options.mastra;
          ...
          const baseContext = {
            ...
            ...chunkLCYW7LNO_cjs.createObservabilityContext({ currentSpan: toolSpan }),
          ...
          if (resumeData) {
            const resumeValidation = chunkMNNNQBVN_cjs.validateToolInput(resumeSchema, resumeData, options.name);
          ...
          result = await chunkAHAX7NUG_cjs.executeWithContext({ span: toolSpan, fn: async () => tool?.execute?.(args, toolContext) });
        }
        if (suspendData) {
          const suspendSchema = this.getSuspendSchema();
          const suspendValidation = chunkMNNNQBVN_cjs.validateToolSuspendData(suspendSchema, suspendData, options.name);
        ...
        if (chunkMNNNQBVN_cjs.isVercelTool(tool)) {
          ...
          const outputValidation = chunkMNNNQBVN_cjs.validateToolOutput(outputSchema, result, options.name, false);
    ...
    return async (args, execOptions) => {
      ...
      const toolSpan = chunkAHAX7NUG_cjs.getOrCreateSpan({
      ...
      if (fgaProvider) {
        const { getAgentToolFGAResourceId, getMCPToolFGAResourceId, getStandaloneToolFGAResourceId, requireFGA } = await import('./fga-check-HJCCPRLA.cjs');
        ...
        await requireFGA({
          ...
          permission: chunkFEIEEVTF_cjs.MastraFGAPermissions.TOOLS_EXECUTE,
          requestContext: toolRequestContext,
          actor: execOptions?.actor,
          context: {
            resourceId: options.resourceId
      ...
      try {
        ...
        if (!isResuming) {
          const { data, error } = chunkMNNNQBVN_cjs.validateToolInput(parameters, args, options.name);
        ...
        const mastraError = new chunkZWNRICII_cjs.MastraError(
          {
            id: "TOOL_EXECUTION_FAILED",
            domain: chunkZWNRICII_cjs.ErrorDomain.TOOL,
  ...
  build() {
    ...
    if (chunkMNNNQBVN_cjs.isVercelTool(this.originalTool) && "needsApproval" in this.originalTool) {
    ...
    const instanceNeedsApprovalFn = chunkMNNNQBVN_cjs.getNeedsApprovalFn(this.originalTool);
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunkMNNNQBVN_cjs.Tool) && !chunkMNNNQBVN_cjs.isVercelTool(tool)) {
...
//# sourceMappingURL=chunk-HEE7C2GC.cjs.map
//# sourceMappingURL=chunk-HEE7C2GC.cjs.map
...
var chunkG4Y6R4NY_cjs = require('./chunk-G4Y6R4NY.cjs');
      ...
      for (const part of sourceParts) {
        if (part.type === `file`) {
          ...
          if (typeof fileData === "string") {
            ...
            normalizedUrl = imageContentToString(fileData, fileMimeType);
          }
          ...
          const toolInvocation = {
            ...
            } : isDeniedApproval ? { result: part.toolInvocation.approval?.reason ?? "Tool call was not approved by the user" } : {}
          };
          let currentStep = -1;
          let toolStep = -1;
...
var AIV5Adapter = class {
    ...
    if (dbMsg.content.parts) {
      for (const part of dbMsg.content.parts) {
        ...
        if (part.type === "file") {
          ...
          const categorized = typeof fileData === "string" ? categorizeFileData(fileData, fileMimeType) : { type: "raw", mimeType: fileMimeType};
          if (categorized.type === "url" && typeof fileData === "string") {
            ...
            if (typeof fileData === "string") {
              ...
              filePartData = imageContentToString(fileData, extractedMimeType);
            }
  ...
  static fromUIMessage(uiMsg) {
    ...
    const toolInvocationParts = parts.filter((p) => chunkHNRFQPHO_cjs.isToolUIPart(p));
    ...
    const v2Parts = parts.map((p) => {
      if (chunkHNRFQPHO_cjs.isToolUIPart(p)) {
        const toolName = getToolName(p);
...
var AIV6Adapter = class _AIV6Adapter {
  static toUIMessage(dbMsg) {
    ...
    if (!hasToolInvocationParts || !hasReasoningParts || !hasFileParts || !hasTextParts) {
      for (const part of v5Message.parts) {
        if (chunkHNRFQPHO_cjs.isToolUIPart(part)) {
  ...
  static fromUIMessage(uiMsg) {
    const compatibleParts = uiMsg.parts.filter((part) => {
      ...
      if (chunk4MB5NE52_cjs.isToolUIPart(part)) return false;
      return true;
    });
    ...
    for (const part of uiMsg.parts) {
      ...
      if (!chunk4MB5NE52_cjs.isToolUIPart(part)) {
...
var CacheKeyGenerator = class _CacheKeyGenerator {
  ...
  static fromAIV5Parts(parts) {
    ...
    for (const part of parts) {
      ...
      if (chunkHNRFQPHO_cjs.isToolUIPart(part) || part.type === "dynamic-tool") {
        key += part.toolCallId;
        key += part.state;
      }
      if (part.type === "reasoning") {
...
function sanitizeV5UIMessages(messages, filterIncompleteToolCalls = false) {
  ...
  const getSafeParts = (m, assistantTurnStillOpen) => m.parts.filter((p) => {
    ...
    if (!chunkHNRFQPHO_cjs.isToolUIPart(p)) return true;
  ...
  const msgs = messages.map((m, idx) => {
    ...
    const sanitized = {
      ...
      parts: mergedParts.map((part) => {
        if (chunkHNRFQPHO_cjs.isToolUIPart(part) && part.state === "output-available") {
          return {
            ...part,
            output: (() => {
              const o = part.output;
...
function addStartStepPartsForAIV5(messages) {
  for (const message of messages) {
    ...
    for (const [index, part] of message.parts.entries()) {
      if (!chunkHNRFQPHO_cjs.isToolUIPart(part)) continue;
      const nextPart = message.parts.at(index + 1);
      if (nextPart && nextPart.type !== `step-start` && !chunkHNRFQPHO_cjs.isToolUIPart(nextPart)) {
        message.parts.splice(index + 1, 0, { type: "step-start" });
      }
      if (nextPart && chunkHNRFQPHO_cjs.isToolUIPart(nextPart) && !part.providerExecuted && nextPart.providerExecuted && (nextPart.state === "output-available" || nextPart.state === "output-error")) {
        message.parts.splice(index + 1, 0, { type: "step-start" });
      }
    }
  }
...
function aiV4UIMessagesToAIV4CoreMessages(messages) {
  return chunkG4Y6R4NY_cjs.convertToCoreMessages(sanitizeAIV4UIMessages(messages));
}
...
function aiV5UIMessagesToAIV5ModelMessages(messages, dbMessages, filterIncompleteToolCalls = false) {
  ...
  for (const uiMsg of preprocessed) {
    const produced = chunkHNRFQPHO_cjs.convertToModelMessages([uiMsg]);
...
var StepContentExtractor = class _StepContentExtractor {
  ...
  static convertPartsToContent(parts, stepId, stepContentFn) {
    ...
    const modelMessages = chunkHNRFQPHO_cjs.convertToModelMessages(sanitizeV5UIMessages(stepUiMessages));
...
function convertImageFilePart(part, downloadedAssets) {
  ...
  const { data: convertedData, mediaType: convertedMediaType } = chunkDVDSGBUW_cjs.convertToDataContent(originalData);
...
exports.resolveFilePartMediaTypeAndData = resolveFilePartMediaTypeAndData;
...
//# sourceMappingURL=chunk-ORSCZRC3.cjs.map
//# sourceMappingURL=chunk-ORSCZRC3.cjs.map